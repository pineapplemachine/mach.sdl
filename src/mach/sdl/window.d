...
class Window{
    ...
    this(
        ...
        if(!this.context){
            throw new SDLException("Failed to create SDL_GLContext.");
        }
        //GL.reload();
        //GL.Version.verify();
        GL.initialize();
        ...
        this.vsync(vsync);
        this.register();
        GLException.enforce("Failed to initialize window.");
    }
    ...
    @property void viewport(N)(in Box!N screen){
        //glViewport(screen.x, screen.y, screen.width, screen.height);
        GLException.enforce("Failed to set viewport.");
    }
