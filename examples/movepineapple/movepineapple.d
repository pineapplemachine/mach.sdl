...
struct Render{
    ...
    GLShader shader(in GLShader shader){
        this.program.add(shader);
    }
    GLShader shader(in GLShader.Type type, in string source){
        return this.shader(GLShader(type, source));
    }
    GLShader fragment(in string source){
        return this.shader(GLShader.Type.Fragment, source);
    }
    GLShader vertex(in string source){
        return this.shader(GLShader.Type.Fragment, source);
    }
}
